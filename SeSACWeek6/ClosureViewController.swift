...
class ClosureViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        /*
         오토리사이징을 오토 레이아웃 제약조건처럼 설정해주는 기능이 내부적으로 구현되어 있음
         이 기능은 디폴트가 true, 하지만 오토 레이아웃을 지정해주면 오토리사이징을 안 쓴다는 의미로 false로 상태가 내부적으로 변경됨
         오토리사이징 + 오토레이아웃 = 충돌
         코드 기반 UI > true
         인터페이스 빌더 기반 오토레이아웃을 모두 설정한 UI > true
        ...
        cardView.posterImageView.backgroundColor = .green
